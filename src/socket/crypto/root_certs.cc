/* MSVC doesn't support C11 stdatomic.h properly yet.
...
int v_no_password_callback(char *buf, int size, int rwflag, void *u) {
    return 0;
}

static X509 *v_ssl_ctx_get_X509_without_callback_from(struct v_cert_string content) {
    X509 *x = NULL;
    BIO *in;
    ...
    x = PEM_read_bio_X509(in, NULL, v_no_password_callback, NULL);
    if (x == NULL) {
...
static void v_internal_init_root_certs() {
    ...
    if (!atomic_exchange(&root_cert_instances_initialized, 1)) {
        for (size_t i = 0; i < root_certs_size; i++) {
            root_cert_instances[i] = v_ssl_ctx_get_X509_without_callback_from(root_certs[i]);
        }
    }
...
extern "C" int v_internal_raw_root_certs(struct v_cert_string **out) {
    *out = root_certs;
    return root_certs_size;
}

extern "C" X509_STORE *v_get_default_ca_store() {
